...
>- exp の値は 33259303656（Unixタイムスタンプ）
...
```
swagger-ui.instant.htbにアクセスしてみる
...
>- /home/shirohige/.ssh/探索
```
sshd:x:104:65534::/run/sshd:/usr/sbin/nologin
```
があるので
SSH サーバー (sshd) が動作している可能性が高い
→もし shirohige のパスワードを見つければ SSH でログインできる
...
## 権限昇格
低レベルユーザなので/rootディレクトリのルート フラグを読み取ることができない。そのため、権限を昇格するか、ルート ユーザーの資格情報を取得する方法を見つける
色々探索していたら/optの中にbackuosをみつけた
```
...
```
.datが暗号化されているのでcracked.pyを作る（以下を参考にした）
...
root@10.10.11.37's password: 
```
なぜかできなかったので`shirohige`でsshログインし、上記のパスワードを使用してユーザーを root に切り替え
```
┌──(venv)─(kali㉿kali)-[~/htb/Machines/Instant]
└─$ ssh shirohige@10.10.11.37 -i id_rsa                                                         
Welcome to Ubuntu 24.04.1 LTS (GNU/Linux 6.8.0-45-generic x86_64)
